describe('Criação de Notícias', () => {
  const categories = [
    { 
      ...
      excerpt: 'Setor de tecnologia registra crescimento de 30% nas contratações no último trimestre, com destaque para vagas de desenvolvedores e analistas de dados.',
      content: 'O mercado de trabalho brasileiro mostra sinais de recuperação significativa, especialmente no setor de tecnologia. Segundo dados do Ministério do Trabalho, foram abertas mais de 50 mil vagas apenas no último trimestre, representando um crescimento de 30% em relação ao mesmo período do ano anterior. As principais oportunidades estão concentradas em desenvolvimento de software, análise de dados e inteligência artificial.',
    ...
    { 
      ...
      name: 'Sociedade',
      title: 'Programa social beneficia 2 milhões de famílias em situação de vulnerabilidade',
      slug: 'programa-social-beneficia-familias-vulnerabilidade',
      excerpt: 'Iniciativa governamental distribuiu R$ 500 milhões em auxílios emergenciais para famílias de baixa renda em todo o país.',
      content: 'O programa social "Auxílio Emergencial Plus" alcançou um marco importante ao beneficiar mais de 2 milhões de famílias em situação de vulnerabilidade social. O investimento de R$ 500 milhões foi distribuído em parcelas de R$ 250 para cada família cadastrada. A iniciativa tem como objetivo reduzir a desigualdade social e garantir condições básicas de sobrevivência durante o período de crise econômica.',
    ...
    { 
      ...
      name: 'Tecnologia',
      title: 'Nova inteligência artificial revoluciona diagnóstico médico com 95% de precisão',
      slug: 'ia-revoluciona-diagnostico-medico-precisao',
      excerpt: 'Sistema desenvolvido por pesquisadores brasileiros consegue identificar doenças em exames de imagem com precisão superior à de especialistas humanos.',
  ...
  categories.forEach((category) => {
    it(`deve criar uma notícia de ${category.name} e publicá-la imediatamente`, () => {
      ...
      cy.get('[data-cy="image-url-input"]').invoke('val', category.image).trigger('input')
      cy.get('input[name="title"]').invoke('val', category.title).trigger('input')
      ...
      cy.get('[data-cy="editorjs-content"] .ce-paragraph', { timeout: 5000 }).should('be.visible')
      cy.get('[data-cy="editorjs-content"] .ce-paragraph').first().type(category.content)
